...
from .utils import (initialize_vocabulary, sentence_to_token_ids,
                    smile_tokenizer, EOS_ID, PAD_ID, GO_ID)
...
class Seq2SeqModel(object):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(
            self,  # pylint: disable=too-many-locals, too-many-arguments, super-init-not-called
            source_vocab_size,
            target_vocab_size,
            buckets,
            size,
        ...
        if not forward_only:
            self.gradient_norms = []
            self.updates = []
            lr_summary_op = tf.summary.scalar("learning rate",
            ...
            for b in xrange(len(buckets)):
                ...
                self.summary_ops.append(
                    tf.summary.merge([
                        tf.summary.scalar(
                            "global_norm_%d" % b, norm), tf.summary.scalar(
                                "loss_%d" % b, self.losses[b]), lr_summary_op
                    ]))
        self.saver = tf.train.Saver(tf.global_variables())

        default_graph = tf.get_default_graph()
        print("\n".join([
            a.name for a in default_graph.get_operations()
            if "gru_cell/add_" in a.name
        ]))
...
#
    ...
    def load_model_from_files(cls,
        ...
        if ckpt:
            print("Loading model weights from checkpoint_dir: %s" %
                  checkpoint_dir)
    ...
    def load_model_from_dir(cls, train_dir, forward_only, sess=None):
        ...
        return cls.load_model_from_files(model_file, checkpoint_dir,
                                         forward_only, sess)

    def save_model_to_files(self,
                            model_file,
                            checkpoint_file,
                            sess=None,
                            verbose=False):
        ...
        if os.path.exists(checkpoint_dir):
            ...
            print("Skip save weights to %s since the dir does not exist." %
                  checkpoint_dir)
    ...
    def save_model_to_dir(self, train_dir, sess=None, verbose=False):
        ...
        self.save_model_to_files(
            model_file, checkpoint_file, sess=sess, verbose=verbose)
    ...
    def step(
            ...
            encoder_inputs,
            decoder_inputs,
            target_weights,  # pylint: disable=too-many-locals, too-many-arguments, too-many-branches, arguments-differ
            bucket_id,
            forward_only,
        ...
        if not forward_only:
            ...
            return outputs[1], outputs[
                2], summary  # Gradient norm, loss, no outputs.
...
class FingerprintFetcher(object):
    ...
    def __init__(self, model_dir, vocab_path, sess=None):
        ...
        self.model = Seq2SeqModel.load_model_from_dir(self.model_dir, True,
                                                      sess)
    ...
    def decode(self, smile_string, sess=None):  # pylint: disable=too-many-locals
        ...
        sess = sess or tf.get_default_session()
        _, _, output_logits, fps = self.model.step(
