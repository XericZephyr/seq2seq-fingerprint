        ...
        while True:
            ...
            if current_step % FLAGS.steps_per_checkpoint == 0:
                ...
                for bucket_id in xrange(len(buckets)):
                    ...
                    def print_em_acc(encoder_inputs, output_logits):
                        """Output EM accuracy."""
                        ...
                        output_op = tf.argmax(output_logits, axis=2)
                        ...
                        # sentences = sess.run(input_op,
                        #                      feed_dict={input_ph: encoder_inputs})
                        # pred_sentences = sess.run(eos_op,
                        #                           feed_dict={output_ph: output_logits})
                        # print(sentences[:, 0])
                        ...
                        em_acc, summary = sess.run(
                            [em_acc_op,
                             tf.summary.scalar("EM_Accuracy_%d" % bucket_id, em_acc_op)],
                            feed_dict={
                                input_ph: encoder_inputs,
                                output_ph: output_logits})

                        return em_acc, summary

                    em_acc, summary = print_em_acc(np.array(encoder_inputs),
                                                   np.array(output_logits))
                    if FLAGS.summary_dir:
